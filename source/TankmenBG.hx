...
class TankmenBG extends FlxSprite {
	...
	override public function update(elapsed:Float) {
		...
		if (animation.curAnim.name == 'run') {
			...
			else x = wackyShit - (Conductor.songPosition - strumTime) * tankSpeed;
		}
		...
		if (animation.curAnim.name == 'shot' && animation.curAnim.curFrame >= animation.curAnim.frames.length - 1) kill();
	}
}